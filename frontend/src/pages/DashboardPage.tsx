...
import { getAllPatients, getPredictionHistory } from "../api/predictionApi";
  ...
  async function loadDashboard() {
    try {
      ...
      for (const patient of patientsData) {
        const history = await getPredictionHistory(patient.id);
        count += history.length;

        for (const pred of history) {
          if (
            !latest ||
            new Date(pred.createdAt) > new Date(latest.createdAt)
          ) {
            latest = pred;
          }
        }
    ...
    <div className="min-h-screen bg-[#0b0e14] text-[#e1e2eb] font-[Inter] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1500px] mx-auto p-8">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          {/* Latest Result */}
          <div className="col-span-12 md:col-span-4 p-6 rounded-2xl bg-[#1d2026]/60 backdrop-blur-xl border border-[#2a2f3a] hover:scale-[1.02] transition">
            <p className="text-sm text-[#8c90a0] mb-2">Latest Result</p>
            ...
            {latestPrediction ? (
              ...
              <p className="text-[#555]">No data</p>
            )}
          </div>
